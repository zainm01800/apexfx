...
// Intraday is shallow on purpose — surfaced in the UI so thin samples are obvious.
const MAX_DAYS = { '1m': 7, '5m': 60, '15m': 60, '30m': 60, '1h': 729, '4h': 729, '1d': 3649, '1w': 3649 };
const TIMEFRAMES = ['1m', '5m', '15m', '30m', '1h', '4h', '1d', '1w'];

const yahooProvider = {
  name: 'yahoo',
  maxDays: (tf) => MAX_DAYS[tf] || 3649,
...
let _provider = yahooProvider;
function setProvider(p) { _provider = p; }
function getProvider() { return _provider; }
function getMaxHistory(tf) { return _provider.maxDays(tf); }
function getCandles(sym, type, tf, opts) { return _provider.getCandles(sym, type, tf, opts || {}); }
// Weekly context bars for the confluence strategy on 1d.
function getWeekly(sym, type, opts) { return _provider.getCandles(sym, type, '1w', opts || {}); }

const _datasource = { TIMEFRAMES, MAX_DAYS, yahooProvider, setProvider, getProvider, getMaxHistory, getCandles, getWeekly };
