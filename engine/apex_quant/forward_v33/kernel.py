"""V33 one-cash slow-monthly/fast-five-session synthetic-CFD research replay.
...
from .cost_filter import select_cost_aware

PROFILES={
    'lower':dict(daily=.03,maximum=.06,original_maximum=.06,instrument=.0075,aggregate=.0225,gross=2.,name=.75),
    'higher':dict(daily=.05,maximum=.10,original_maximum=.10,instrument=.01,aggregate=.03375,gross=2.,name=.75),
}
FAST_PROFILES={'lower':dict(aggregate=.0075,gross=.75,name=.25),
               'higher':dict(aggregate=.01275,gross=1.,name=.375)}
...
class ReplayConfig:
    ...
    def __post_init__(self):
        ...
        if self.fast_stop_atr!=2.5 or self.fast_holding_sessions!=5:raise ValueError('Invalid fixed V33 fast stop/holding duration')
...
def replay(panel:Mapping,fx:pd.DataFrame,weights:Mapping,entries:Mapping,atrs:Mapping,cfg:ReplayConfig,*,slow_symbols,fast_symbols,slow_execution_by_session=None)->ReplayResult:
    ...
    for day in active:
        ...
        if not (halted or blocked or fast_open_full) and previous in entries and len(fast_held)<4:
            selected=select_cost_aware(entries[previous],fast_held,{s:float(arr[s][i-1,3]) for s in entries[previous]},atr[previous])
