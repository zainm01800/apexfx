...
def read(client):
    ...
    if response.status_code!=200:raise RuntimeError(f"Authoritative V27B read failed: HTTP {response.status_code}")
    rows=response.json()
    ...
    if not isinstance(payload,dict) or payload.get("book_id")!="v27b":raise RuntimeError("Invalid V27B payload")
...
def write(client,payload,previous):
    ...
    if previous is None:
        if state["revision"]!=1 or state["parent_state_sha256"] is not None:raise RuntimeError("Nonroot V27B initialization")
        row=dict(id=SPEC.runtime_id,user_id="apex_engine",symbol="BOOK_V27B_FORWARD",timeframe="1d",direction="paper",
            feature_vector=payload,analysis_text="V27B joint portfolio: GBP100k forward paper only",verdict="EXPERIMENTAL_FORWARD_PAPER")
        ...
        if state["parent_state_sha256"]!=digest(previous) or state["revision"]!=previous["revision"]+1:
            raise RuntimeError("Invalid V27B write lineage")
    ...
    if response.status_code not in (200,201):raise RuntimeError(f"V27B conditional write failed: HTTP {response.status_code}")
    rows=response.json()
    if not isinstance(rows,list) or len(rows)!=1:raise RuntimeError("V27B stale writer rejected; state was not overwritten")
    saved=read(client)
    if saved.status!="found" or digest(saved.payload["state"])!=digest(state):raise RuntimeError("V27B durable read-back did not match")
