...
def new_state(now=None):
    ...
    return dict(book_id="v27b",spec_sha256=SPEC_HASH,revision=1,parent_state_sha256=None,
...
def validate_state(state,spec=SPEC):
    if not isinstance(state,dict) or state.get("book_id")!="v27b" or state.get("spec_sha256")!=SPEC_HASH:
        raise ValueError("V27B state identity/specification mismatch")
    if state.get("initial_equity")!=100000 or not isinstance(state.get("revision"),int) or state["revision"]<1:
        raise ValueError("V27B cash seed/revision mismatch")
    for key in ("bars","fx","atrs","decisions"):
        if not isinstance(state.get(key),dict):raise ValueError(f"V27B malformed {key}")
    for key in ("positions","trades","daily","events"):
        if not isinstance(state.get(key),list):raise ValueError(f"V27B malformed {key}")
...
def _enrich(lot,state,mark=None,fx=None):
    ...
    row.update(instrument=row["symbol"],decision_recorded_at_utc=record.get("recorded_at_utc"),
        ...
        signal_rationale="Top-three monthly relative strength; inverse-volatility allocation and 10-month trend filter" if row["sleeve"]=="slow" else "Negative five-session beta-adjusted stock return; known filing and large-gap exclusions passed")
...
def _rebuild(state):
    ...
    for stamp,row in result.daily.iterrows():
        ...
        daily.append(dict(**raw,date=stamp.isoformat(),equity_gbp=equity,cash_gbp=cash,
            external_daily_floor=raw["external_daily_floor_gbp"],external_maximum_floor=88000.,
    ...
    if abs(state["equity_gbp"]-100000-closed-opened)>1e-6:raise ValueError("V27B P&L reconciliation failed")
...
def public_payload(state,now=None):
    ...
    daily=state["daily"] or [dict(date=state["activated_at_utc"],equity_gbp=100000.,cash_gbp=100000.,
        day_pnl_gbp=0.,open_pnl_gbp=0.,drawdown_from_peak=0.,is_seed=True,external_daily_floor=95000.,external_maximum_floor=88000.)]
    metadata=dict(book_id="v27b",profile=CONTRACT["profile"],label=SPEC.label,account_currency="GBP",initial_equity=100000,
    ...
    return clean(dict(schema_version=1,book_id="v27b",generated_at_utc=now.isoformat(),metadata=metadata,
