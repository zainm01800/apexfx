...
| 2026-07-19 | **#3: `max_risk_per_trade` 0.02 → 0.01 (engine/config.yaml)** | Prop-rules Monte Carlo sweep (`engine/scratch/prop_risk_sweep.py`, 20k paths/level, Book H gold trade stats, static-floor two-step rules): 1.0% is the survival-adjusted optimum for the funded-account goal — ~82–94% pass/attempt, ~97–99.6% cumulative with one retry inside 6 months, ~89–97% funded 12-month survival; 1.5–2.0% trades survival for speed unfavourably, 0.5–0.75% is too slow for the 6-month window. The paper proof should validate the exact configuration a funded account would run. Sizing change only — universe, signals, exits, other caps untouched; most trades already size below 1% via vol-scaling/kelly-fraction, so only the largest high-conviction positions are clipped. | **NO — the book is unchanged; sizing is a deployment parameter, and the whole point of the proof is the deployed configuration** |
