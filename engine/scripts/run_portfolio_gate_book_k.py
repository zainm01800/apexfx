...
Prereg: engine/data_store/book_k_prereg.md (2026-07-22). Universe-only
...
exactly TWO configs, one variable.
  ...
  book_k_indep_252  baseline + 24 screened large caps                     (1 NEW charge)
...
Usage:
    ...
    .venv-mac/bin/python scripts/run_portfolio_gate_book_j.py --out data_store/validation/book_k_gate_run2.json
...
DEFAULT_RESULTS_PATH = ENGINE_DIR / "data_store" / "validation" / "book_k_gate_2026-07-22.json"

# ── The 12 independence-selected additions (prereg §2) ───────────────────────
ADDITIONS_12 = [
    # Selection rule (prereg §2, mechanical + ex-ante, NO performance data):
    #   1. correlate each of the 59 screened candidates against EVERY book instrument
    #   2. reject any with max|corr| >= 0.50 to an existing holding (near-duplicate) —
    #      this removes 22 of 59, incl. every semi (KLAC .895 vs SOXX) and every
    #      energy name (COP .869, CVX .859 vs XLE). Book J added XOM (.885 vs XLE)
    #      and CVX, i.e. clones that cost turnover and add ~no breadth.
    #   3. rank survivors by mean|corr| ascending, take 12.
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("=" * 72, flush=True)
    print(f"PORTFOLIO GATE (BOOK K — INDEPENDENCE) 2026-07-22 | mode=ITERATION "
    ...
    out = {
        ...
        "prereg": "engine/data_store/book_k_prereg.md",
