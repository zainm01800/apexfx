#!/usr/bin/env python3
"""V27B paper-only runner. Explicit activation, no historical profit import."""
import argparse
...
import httpx
from apex_quant.forward_v27b.data import fetch_market
from apex_quant.forward_v27b.state import new_state, advance, public_payload, digest, enforce_deadline, confirm_durable_decisions
from apex_quant.forward_v27b.storage import read, write
...
def main(argv=None):
    ...
    with httpx.Client(timeout=60) as client:
        ...
        if previous is None and not args.activate:
            print("V27B has not been activated; no account created. Use explicit --activate.")
            return 2
        ...
        print(f"Book V27B | paper only | {result['status']} | equity GBP{result['equity_gbp']:.2f} | "
        ...
        if args.dry_run:print("DRY RUN: no state written")
            ...
            print("Verified V27B atomic paper state saved; no other book changed")
