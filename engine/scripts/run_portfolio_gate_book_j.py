...
Prereg: engine/data_store/universe_expansion_prereg.md (2026-07-22). Universe-only
...
exactly TWO configs, one variable.
  ...
  book_j_breadth_252  baseline + 24 screened large caps                     (1 NEW charge)
...
Usage:
    ...
    .venv-mac/bin/python scripts/run_portfolio_gate_book_j.py --out data_store/validation/book_j_gate_run2.json
...
DEFAULT_RESULTS_PATH = ENGINE_DIR / "data_store" / "validation" / "book_j_gate_2026-07-22.json"

# ── The 24 additions (prereg §2) ──────────────────────────────────────────────
# Chosen for sector spread from the 59 screened in halal_screen_2026-07-22.md.
# Deliberately NO extra semis/software: the book is already long that factor via
# AAPL/MSFT/NVDA/AMD/TSM + SMH/SOXX/XLK, so adding more would raise nominal breadth
# while concentrating the existing tilt — the opposite of the hypothesis.
ADDITIONS_24 = [
    "JNJ", "MRK", "PFE", "ABBV", "LLY", "TMO", "ABT", "ISRG",   # healthcare (8)
    "PG", "KO", "PEP", "CL", "MDLZ",                             # staples (5)
    "NKE", "HD", "LOW", "TJX",                                   # discretionary (4)
    "LIN", "UNP", "ITW", "HON", "APD",                           # industrials/materials (5)
    "XOM", "CVX",                                                # energy (2)
]
...
def main(argv: list[str] | None = None) -> int:
    ...
    for inst in wanted:
        ...
        if df.empty:
            if inst in ADDITIONS_24:
                missing_additions.append(inst)
    ...
    print("=" * 72, flush=True)
    print(f"PORTFOLIO GATE (BOOK J — BREADTH) 2026-07-22 | mode=ITERATION "
    ...
    if missing_additions:
        print(f"  WARNING: {len(missing_additions)} addition(s) lacked usable data and are "
              f"NOT in the challenger: {', '.join(sorted(set(missing_additions)))}")
    ...
    print(f"  DECISION: {'ADOPT the expansion' if adopt else 'ADOPT NOTHING — certified book stands'}")
    print("=" * 72, flush=True)

    out = {
        "generated_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "mode": "iteration", "holdout_start": args.holdout_start,
        "prereg": "engine/data_store/universe_expansion_prereg.md",
        "universes": {name: list(p.keys()) for name, p in panels.items()},
        "missing_additions": sorted(set(missing_additions)),
