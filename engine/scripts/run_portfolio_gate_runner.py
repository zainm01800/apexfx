...
Prereg: engine/data_store/runner_exit_prereg.md (2026-07-22). Universe-only
...
exactly TWO configs, one variable.
  ...
  book_runner_252  baseline + 24 screened large caps                     (1 NEW charge)
...
Usage:
    ...
    .venv-mac/bin/python scripts/run_portfolio_gate_book_j.py --out data_store/validation/runner_gate_run2.json
...
DEFAULT_RESULTS_PATH = ENGINE_DIR / "data_store" / "validation" / "runner_gate_2026-07-22.json"

# ── Runner exit (prereg: runner_exit_prereg.md) ──────────────────────────────
# Both configs trade the IDENTICAL universe. The only difference is the exit:
# the baseline caps every winner at the fixed 1.5R target; the runner takes the
# same 50% partial at 1R + breakeven, then lets the remaining half ride the
# existing 2xATR Chandelier trail UNCAPPED. One variable.
...
PANEL_UNIVERSES = {
    ...
    "book_runner_252": GOLD_UNIVERSE,
}
UNIVERSE_LABELS = {
    "book_h_gold_252": "book_h_gold_39",        # must byte-match the 2026-07-19 key -> dedup
    "book_runner_252": "book_h_gold_39_runner",
}

BOOKS = {name: {"carry_filter": False, **COMMON_PARAMS, "momentum_lookback": 252}
         for name in PANEL_UNIVERSES}
# The runner differs by EXIT, not by params/universe — record that in the trial key,
# otherwise its canonical JSON matches the baseline's and the ledger silently dedups
# a genuinely new experiment down to zero charges.
BASELINE = "book_h_gold_252"
CHALLENGER = "book_runner_252"
BOOKS[CHALLENGER] = {**BOOKS[CHALLENGER], "exit_variant": "runner_uncapped_trail"}
#: Baseline uses the default (capped) TradeManager; the challenger rides the trail.
RUNNER_TM = {BASELINE: None, CHALLENGER: TradeManager(runner_mode=True)}
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("=" * 72, flush=True)
    print(f"PORTFOLIO GATE (RUNNER EXIT) 2026-07-22 | mode=ITERATION "
    ...
    for name, params in BOOKS.items():
        ...
        res = PortfolioBacktester(cfg, exit_mode="managed", trade_manager=RUNNER_TM[name]).run(
    ...
    print(f"  DECISION: {'ADOPT the runner exit' if adopt else 'ADOPT NOTHING — capped 1.5R target stands'}")
    print("=" * 72, flush=True)

    out = {
        "generated_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "mode": "iteration", "holdout_start": args.holdout_start,
        "prereg": "engine/data_store/runner_exit_prereg.md",
