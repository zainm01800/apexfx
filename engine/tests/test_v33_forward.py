...
from apex_quant.forward_v33 import state as st
from apex_quant.forward_v33.spec import SYMBOLS, ETFS, CIKS, SPEC, SPEC_HASH
from apex_quant.forward_v33.data import anchored_panel, extract_filings
from apex_quant.forward_v33.features import build_features, build
from apex_quant.forward_v33.storage import write
from apex_quant.forward_v14.data import XNYS, DataUnavailable
from apex_quant.forward_v33.cost_filter import cost_to_stop, filter_scores, select_cost_aware
from apex_quant.forward_v33.spec import CONTRACT
from apex_quant.forward_v33.kernel import PROFILES, FAST_PROFILES


def test_v33_is_separate_strict_ten_percent_cost_aware_contract():
    assert SPEC.book_id == "v33" and SPEC.runtime_id == "__apex_book_v33_forward_paper_runtime__"
    assert CONTRACT["maximum"] == PROFILES["higher"]["maximum"] == .10
    assert CONTRACT["profile"] == "higher_5_10_cost_aware_joint"
    assert FAST_PROFILES["higher"] == dict(aggregate=.01275,gross=1.,name=.375)
    payload=st.public_payload(st.new_state("2026-09-08T15:00Z"))
    assert payload["book_id"] == "v33" and payload["daily"][0]["external_maximum_floor"] == 90000
    assert payload["positions"] == payload["trades"] == []


def test_fixed_prior_close_filter_and_deterministic_ranking():
    expected=(.1+.001*95*.999+.001*95+100*.02*7/365)/5
    assert cost_to_stop(100,2) == pytest.approx(expected)
    assert cost_to_stop(100,.01) > .1
    scores={"A":10,"B":2,"C":2}
    closes={"A":100,"B":100,"C":100};atrs={"A":.01,"B":2,"C":2}
    assert filter_scores(scores,closes,atrs)=={"B":2.,"C":2.}
    assert select_cost_aware(scores,{"B"},closes,atrs)==[("C",2.)]
    for invalid in (0,-1,float("nan"),float("inf")):
        with pytest.raises(ValueError):cost_to_stop(100,invalid)


def test_v27b_account_cannot_be_used_as_v33_even_with_same_seed():
    from apex_quant.forward_v27b.state import new_state as old_state
    with pytest.raises(ValueError,match="identity"):
        st.validate_state(old_state("2026-09-08T15:00Z"))


def test_saved_candidates_carry_prior_close_filter_evidence():
    s=seeded()
    for decision in s["decisions"].values():
        for symbol in decision["fast_entries"]:
            evidence=decision["evidence"][symbol]
            assert evidence["prior_price_date"] == decision["decision_date"]
            assert evidence["stressed_cost_to_stop"] <= .10
            assert evidence["fast_budget_scale"] == .5
