"""Test 4-Sleeve Portfolio Leverage Scaling on 1 Single £100k Account.

Evaluates scaling factors (1.2x, 1.4x, 1.6x, 1.8x) on the 4-sleeve diversified portfolio:
  Sleeve 1: Trend Book Runner (60% weight)
  Sleeve 2: Equity XS Momentum (20% weight)
  Sleeve 3: TOM Seasonality (10% weight)
  Sleeve 4: Crypto XS Momentum (10% weight)
"""
...
def run_scaled_4sleeve_backtest():
    bars = load_bars()
    pits = {inst: PointInTimeAccessor(df) for inst, df in bars.items()}
    
    print("=" * 70)
    print("SCALED 4-SLEEVE PORTFOLIO SWEEP ON 1 SINGLE £100K ACCOUNT")
    print("=" * 70)
    
    # 1. Sleeve 1: Trend Book Runner @ 1.20% risk
    cfg = get_config()
    cfg.risk.max_risk_per_trade = 0.0120
    ...
    base_ret = 0.60 * df_comb["trend"] + 0.20 * df_comb["eq_xs"] + 0.10 * df_comb["tom"] + 0.10 * df_comb["crypto_xs"]
    
    for scale in [1.0, 1.2, 1.4, 1.6, 1.8]:
        p_ret = base_ret * scale
        r = p_ret.to_numpy()
        ...
        print(f"Scale {scale:.1f}x -> Sharpe: {sh:.3f}, Monthly: £{monthly_gbp:.2f}/mo ({monthly_ret_pct*100:.2f}%/mo), MaxDD: {max_dd*100:.2f}%")
    print("=" * 70)


if __name__ == "__main__":
    run_scaled_4sleeve_backtest()
